# -*- coding: utf-8 -*-
"""HRS4 卫星观测算子数据模块 (DataModule)。

本模块为 :class:`HRS4DataModule`，对应 HRS4 (FY-4A GIIRS Hyperspectral Infrared
Sounder) 任务的纯 PyTorch 数据模块实现，无 PyTorch-Lightning 依赖。

任务特点:

- GIIRS 是中国风云四号 A 星搭载的**超高光谱红外探测仪**，通道数达 ~900 (本模块
  1.0° 分辨率预处理后保留 ``tmbrs_value[:, -1]``，即丢弃最末通道)。
- 探测通道覆盖长波 (650~1130 cm⁻¹) 和中波 (1650~2250 cm⁻¹) 两个红外波段，对大气
  温度和湿度廓线敏感。
- 辅助场为 ``fovn`` / ``lsql`` / ``saza`` / ``soza`` / ``hols`` / ``hmsl`` /
  ``solazi`` / ``bearaz`` 共 8 类，与 AMSU-A / MHS 共用 ``hols/hmsl`` scaler。
- ``fovn`` 在 HRS4 中除以 56 (GIIRS 每条扫描线 56 个视场)。

数据布局::

    root_dir/<yyyy>/<yyyy-mm-dd>/<hh:mm:ss>.npy                          # ERA5 状态
    obs_dir/1bhrs4_merged_npy_1.0deg/<yyyy>/<yyyy-mm-dd>/                 # HRS4 观测
        <hh:mm:ss>-auxiliary_value.npy
        <hh:mm:ss>-tmbrs_value.npy       # 加载时通过 [:-1] 丢弃末通道
        <hh:mm:ss>-mask.npy
"""
import os
import numpy as np
...
from src.datamodules.obsoperator.hrs4.npydataset import NpyDataset, collate_fn

class HRS4DataModule:
    """HRS4 观测算子数据模块 (纯 torch 实现，无 PyTorch-Lightning 依赖)。

    负责 ERA5 状态与 HRS4 (GIIRS) 卫星观测 (亮温 + 辅助场) 的训练/验证/测试加载，
    按 ``start_train_year / start_val_year / start_test_year / end_year`` 切分时间窗。
    """
    ...
    def __init__(
        ...
        ``hrs4_1.0deg_schema.json`` 元数据。

        Args:
            root_dir (str): ERA5 等模式数据根目录。
            obs_dir (str): 卫星观测根目录。
            variables (list[str]): ERA5 状态变量列表。
            tmbrs_vars (list[str]): HRS4 亮温变量列表 (丢弃末通道后)。
        ...
        self.tmbrs_mean, self.tmbrs_std = self._load_normalize(f"{obs_dir}/1bhrs4_merged_npy_1.0deg", self.tmbrs_vars)
        ...
        self.hols_scaler = dict(np.load(os.path.join(f"{obs_dir}/1bhrs4_merged_npy_1.0deg", "hols_scaler.npz")))
        self.hmsl_scaler = dict(np.load(os.path.join(f"{obs_dir}/1bhrs4_merged_npy_1.0deg", "hmsl_scaler.npz")))
