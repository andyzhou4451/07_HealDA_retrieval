# -*- coding: utf-8 -*-
"""MHS 卫星观测算子数据集 (Dataset)。

本模块实现 :class:`NpyDataset`，为 MHS (Microwave Humidity Sounder) 的观测算子
任务提供样本加载逻辑：从 ERA5 状态文件与 ``1bmhs_merged_npy_1.0deg`` 下的卫星
辅助场 / 亮温 / 掩码 npy 文件中读取单时间点样本，并对辅助场做角度 → cos 投影、
线性归一化等预处理。
...
class NpyDataset(Dataset):
    """加载 ``.npy`` 格式 ERA5 状态 + MHS 卫星观测的 :class:`Dataset`。
    ...
    def __init__(
        ...
        Args:
            ...
            meta_data (dict): ``mhs_1.0deg_schema.json`` 元数据。
    ...
    def _get_mhs(self, idx):
        """按索引加载并预处理 MHS 单时间点卫星观测。

        关键预处理 (与 AMSU-A 类似，但 ``fovn`` 归一化分母为 90 而非 30)：
        - ``fovn``：除以 90 (MHS 视场数 90)。
        - ``lsql``：除以 2。
        - ``saza / soza``：``cos(deg2rad(.))``。
        - ``hols / hmsl``：min-max 归一化。
    ...
    def __getitem__(self, global_idx):
        ...
        tmbrs_data, auxiliary_data, mask = self._get_mhs(global_idx)
