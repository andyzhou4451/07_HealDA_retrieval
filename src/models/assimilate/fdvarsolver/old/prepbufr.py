...
class Solver(nn.Module):
    def __init__(
        ...
        self.phi_r = phi_r
        obs_err = np.load(os.path.join(obs_dir, "GDAS_prepbufr_corrected_npy_1.0deg_20260211/obs_error_48h_forecast_qc.npz"))
        obs_err_list = []
    ...
    def forward(self, xb, prepbufr, prepbufr_mask, std, out_vars):
        return self.solve(xb, prepbufr, prepbufr_mask, std, out_vars)

    def solve(self, xb, prepbufr, prepbufr_mask, std, out_vars):
        xa = self.solver_step(xb, prepbufr, prepbufr_mask, std, out_vars)

        return xa

    def solver_step(self, xb, prepbufr, prepbufr_mask, std, out_vars):
        var_cost_grad = self.var_cost(xb, prepbufr, prepbufr_mask, std, out_vars)
        
        xa = self.model_DA(xb, var_cost_grad, out_vars, use_checkpoint=True)

        return xa

    def var_cost(self, xb, prepbufr, prepbufr_mask, std, out_vars):
        preds = self.forecast(xb, prepbufr, out_vars)

        dy, prepbufr_mask = self.model_H(
            preds, 
            prepbufr * prepbufr_mask, 
            prepbufr_mask, 
            std,
        )
        ...
        # # 保存直方图到硬盘
        # plt.savefig('/public02/code/XiChen_1.0deg/figures/var_cost_grad/var_cost_grad_prepbufr_histogram.png', dpi=300, bbox_inches='tight')  # 保存为 PNG 格式
        # # plt.savefig('var_cost_grad_histogram.pdf')  # 也可以保存为 PDF 格式
        # plt.close()  # 关闭图形以释放内存

        # del preds, pred_prepbufr, log_var, tgt_prepbufr, dy, normgrad_, loss
