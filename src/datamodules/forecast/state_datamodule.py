# -*- coding: utf-8 -*-
"""
天气预报任务的状态场 (State) 数据模块。

本模块基于纯 PyTorch (无 pytorch-lightning 依赖) 实现 ``StateForecastDataModule``，负责：
    1. 加载 ERA5 状态场的归一化参数 (mean/std)；
    2. 按 ``start_train_year`` / ``start_val_year`` / ``start_test_year`` / ``end_year``
       切片生成 train / val / test 三套 :class:`NpyDataset`；
    3. 在 DDP 场景下挂载 :class:`DistributedSampler`，保证多卡训练时样本不重叠；
    4. 对外暴露与 Lightning ``LightningDataModule`` 同名的 ``train/val/test_dataloader``，
       以便 ``BaseTrainer`` 在 ``main.py`` 中以统一接口实例化使用。

对应的 Dataset 实现在 :mod:`src.datamodules.forecast.state_dataset`，
观测版 (含卫星亮温) 在 :mod:`src.datamodules.forecast.obs_datamodule`。
"""
import os
import numpy as np
import torch
...
from src.datamodules.forecast.state_dataset import NpyDataset, collate_fn_forecast


class StateForecastDataModule:
    """天气预报任务的状态场 DataModule (纯 torch 实现)。

    该类负责把 :class:`NpyDataset` (ERA5 状态场 Dataset) 包装为可训练的 DataLoader，
    并提供 DDP 分布式采样器、worker 随机种子、persistent_workers 等工程配置。
    与 ``obs_datamodule.py`` 的差异：本类只产出 (input, target, lead_time, variables, iter_num, std)，
    不含卫星观测数据；观测版请参考 :class:`StateForecastDataModule` in obs_datamodule.py。

    Attributes:
        root_dir (str): 数据根目录。
        variables (list): 气象变量名列表 (如 ["z", "t", "u", "v", "q", ...])。
