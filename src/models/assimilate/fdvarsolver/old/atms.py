...
class Solver(nn.Module):
    def __init__(
        ...
        self.phi_r = phi_r
        obs_err = np.load(os.path.join(obs_dir, "1batms_merged_npy_1.0deg", "avg_obs_error.npz"))
        obs_err_list = []
    ...
    def var_cost(self, xb, sat, sat_mask, std, out_vars):
        ...
        pred_atms, log_var, tgt_atms = self.model_ObsOp(
        ...
        dy, sat_mask = self.model_H(
            pred_atms.view(B, T, -1, H, W), 
            tgt_atms.view(B, T, -1, H, W), 
            sat_mask.view(B, T, 1, H, W), 
            std,
        )
        ...
        # # 保存直方图到硬盘
        # plt.savefig('/public02/code/XiChen_1.0deg/figures/var_cost_grad/var_cost_grad_atms_histogram.png', dpi=300, bbox_inches='tight')  # 保存为 PNG 格式
        # # plt.savefig('var_cost_grad_histogram.pdf')  # 也可以保存为 PDF 格式
        # plt.close()  # 关闭图形以释放内存

        del preds, pred_atms, log_var, tgt_atms, dy, normgrad_, loss
