# -*- coding: utf-8 -*-
"""AMSU-A 卫星观测算子数据模块 (DataModule)。

本模块为 :class:`AMSUADataModule`，对应 AMSU-A (Advanced Microwave Sounding Unit-A)
任务的纯 PyTorch 数据模块实现，无 PyTorch-Lightning 依赖。

任务特点:

- AMSU-A 是搭载在 NOAA / MetOp 系列卫星上的 15 通道微波温度探测仪 (23~89 GHz)，
  本模块对应 1.0° 分辨率的预处理输出。
- 辅助场为 ``fovn`` / ``lsql`` / ``saza`` / ``soza`` / ``hols`` / ``hmsl`` /
  ``solazi`` / ``bearaz`` 共 8 类。
- 预处理使用 ``hols_scaler.npz`` 与 ``hmsl_scaler.npz`` (轨道高度 / 卫星高度
  ``min/max``) 而非 ATMS 的多质量标志 scaler。

数据布局::

    root_dir/<yyyy>/<yyyy-mm-dd>/<hh:mm:ss>.npy                          # ERA5 状态
    obs_dir/1bamsua_merged_npy_1.0deg/<yyyy>/<yyyy-mm-dd>/                # AMSU-A 观测
...
from src.datamodules.obsoperator.amsua.npydataset import NpyDataset, collate_fn

class AMSUADataModule:
    """AMSU-A 观测算子数据模块 (纯 torch 实现，无 PyTorch-Lightning 依赖)。

    负责 ERA5 状态与 AMSU-A 卫星观测 (亮温 + 辅助场) 的训练/验证/测试加载，按
    ...
    def __init__(
        ...
        ``amsua_1.0deg_schema.json`` 元数据，并按 train/val/test 三个时间窗预留
        :class:`NpyDataset` 实例 (懒初始化)。

        Args:
            root_dir (str): ERA5 等模式数据根目录。
            obs_dir (str): 卫星观测根目录。
            variables (list[str]): ERA5 状态变量列表。
            tmbrs_vars (list[str]): AMSU-A 亮温变量列表 (典型 15 通道)。
        ...
        self.tmbrs_mean, self.tmbrs_std = self._load_normalize(f"{self.obs_dir}/1bamsua_merged_npy_1.0deg", self.tmbrs_vars)
        ...
        self.hols_scaler = dict(np.load(os.path.join(f"{self.obs_dir}/1bamsua_merged_npy_1.0deg", "hols_scaler.npz")))
        self.hmsl_scaler = dict(np.load(os.path.join(f"{self.obs_dir}/1bamsua_merged_npy_1.0deg", "hmsl_scaler.npz")))
        with open(f"{self.obs_dir}/1bamsua_merged_npy_1.0deg/amsua_1.0deg_schema.json", "r", encoding="utf-8") as f:
    ...
    def _worker_init_fn(self, worker_id):
        """DataLoader worker 初始化函数 (同 ATMS)。"""
