...
class Solver(nn.Module):
    def __init__(
        ...
        self.phi_r = phi_r
        obs_err = np.load(os.path.join(obs_dir, "satwnd_corrected_npy_1.0deg/obs_error_48h_forecast_qc.npz"))
        obs_err_list = []
        ...
        self.dt = dt
        self.obs_indexes = self.find_obs_indices(all_vars, obs_vars)

    def find_obs_indices(self, all_vars, obs_vars, default=-1):
        var_map = {var: idx for idx, var in enumerate(all_vars)}

        return np.asarray([var_map.get(item, default) for item in obs_vars])

    def forward(self, xb, satwnd, satwnd_mask, std, out_vars):
        return self.solve(xb, satwnd, satwnd_mask, std, out_vars)

    def solve(self, xb, satwnd, satwnd_mask, std, out_vars):
        xa = self.solver_step(xb, satwnd, satwnd_mask, std, out_vars)

        return xa

    def solver_step(self, xb, satwnd, satwnd_mask, std, out_vars):
        var_cost_grad = self.var_cost(xb, satwnd, satwnd_mask, std, out_vars)
        
        xa = self.model_DA(xb, var_cost_grad, out_vars, use_checkpoint=True)

        return xa

    def var_cost(self, xb, satwnd, satwnd_mask, std, out_vars):
        preds = self.forecast(xb, satwnd, out_vars)

        preds_ = preds[:, :, torch.from_numpy(self.obs_indexes).to(preds.device)]

        dy, satwnd_mask = self.model_H(
            preds_, 
            satwnd * satwnd_mask, 
            satwnd_mask, 
            std,
        ...
        # # 保存直方图到硬盘
        # plt.savefig('/public02/code/XiChen_1.0deg/figures/var_cost_grad/var_cost_grad_satwnd_histogram.png', dpi=300, bbox_inches='tight')  # 保存为 PNG 格式
        # # plt.savefig('var_cost_grad_histogram.pdf')  # 也可以保存为 PDF 格式
        # plt.close()  # 关闭图形以释放内存

        # del preds, pred_satwnd, log_var, tgt_satwnd, dy, normgrad_, loss
