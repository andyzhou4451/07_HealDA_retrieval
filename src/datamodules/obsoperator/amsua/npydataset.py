# -*- coding: utf-8 -*-
"""AMSU-A 卫星观测算子数据集 (Dataset)。

本模块实现 :class:`NpyDataset`，为 AMSU-A 的观测算子任务提供样本加载逻辑：从 ERA5
状态文件与 ``1bamsua_merged_npy_1.0deg`` 下的卫星辅助场 / 亮温 / 掩码 npy 文件中
读取单时间点样本，并对辅助场做角度 → cos 投影、线性归一化等预处理。
...
class NpyDataset(Dataset):
    """加载 ``.npy`` 格式 ERA5 状态 + AMSU-A 卫星观测的 :class:`Dataset`。
    ...
    def __init__(
        ...
        Args:
            ...
            tmbrs_vars (list[str]): AMSU-A 亮温变量列表。
            ...
            meta_data (dict): ``amsua_1.0deg_schema.json`` 元数据。
    ...
    def _get_amsua(self, idx):
        """按索引加载并预处理 AMSU-A 单时间点卫星观测。

        关键预处理：
        - ``fovn``：除以 30 归一化 (AMSU-A 视场数 30)。
        - ``lsql``：除以 2 归一化 (扫描质量等级)。
        - ``saza / soza``：``cos(deg2rad(.))``。
        - ``hols / hmsl``：min-max 归一化到 ``[0, 1]``。
        - ``solazi / bearaz``：``cos(deg2rad(./2))`` 消除 0/360 跳变。
        - 当 mask 文件不存在时填充 NaN + 全 0 掩码。
    ...
    def __getitem__(self, global_idx):
        ...
        负索引按 Python 习惯转换为正索引；将 ``_get_amsua`` 中的 NaN 占位替换为 0，
        ...
        tmbrs_data, auxiliary_data, mask = self._get_amsua(global_idx)
...
def collate_fn(batch):
    """AMSU-A DataLoader 的默认 ``collate_fn``。
